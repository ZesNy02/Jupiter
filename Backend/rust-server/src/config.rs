...
impl Config {
  ...
  pub fn load_from_env() -> Config {
    dotenv().ok();
    let _ = env::var("LLM_TOKEN").expect("Couldn't find LLM_TOKEN in environment variables");
    let _ = env
      ::var("CLIENT_SECRET")
      .expect("Couldn't find CLIENT_SECRET in environment variables");
    let _ = env::var("CLIENT_ID").expect("Couldn't find CLIENT_ID in environment variables");
